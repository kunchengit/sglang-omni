...
class Client:
    ...
    async def generate(
        ...
        if request.stream:
            _validate_image_streaming(request)
...
def _validate_image_streaming(request: GenerateRequest) -> None:
    modalities = (
...
def _validate_stream_chunk(data: Any, *, modality: str | None = None) -> None:
    outer_image_modality = modality in {"image", "interleaved"}
    if isinstance(data, dict):
