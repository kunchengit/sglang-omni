...
class ZImageTransformer2DModelWrapper(nn.Module):
    ...
    Args:
        ...
        parallel_runtime: Validated caller-owned groups for native decoding.
    ...
    def __init__(
        ...
        *,
        ...
        parallel_runtime: SGLangDecoderRuntime | None = None,
    ) -> None:
        super().__init__()
        ...
        if backend == "sglang":
            self.runtime = parallel_runtime or SGLangDecoderRuntime(
                DecoderParallelConfig(backend=backend), device, dtype
            )
    ...
    def _load_sglang_model(self, decoder_dir, device, dtype):
        ...
        if (
            self.cfg["n_heads"] % c.ulysses_degree
            or self.cfg["n_kv_heads"] != self.cfg["n_heads"]
        ):
            raise ValueError(
        ...
        try:
            attention = AttentionBackendEnum[c.attention_backend.upper()]
        except KeyError as exc:
            raise ValueError(
                f"Unknown decoder attention backend: {c.attention_backend}"
            ) from exc
        arch = dict(self.cfg)
    ...
    def _native_forward(self, x, t, cap_feats, patch_size, f_patch_size):
        ...
        if self._native_cache is None or self._native_cache[0] != key:
            with self.runtime.preparation("native input preparation"):
                ...
                if full.shape[2] != 1:
                    raise ValueError(
                        "Native spatial decoder supports only one image frame"
                    )
                ...
                # Cache geometry/RoPE, not a previous request's semantic tensors.
                ...
                if (
                    target is not None
                    and target * self.runtime.config.sp_size
                    != ((full_tokens + 31) // 32) * 32
                ):
                    raise ValueError(
                        "Native SP layout would change the SP1 learned-padding token count"
                    )
        ...
        # Native forward returns -velocity, and does not gather spatial output.
