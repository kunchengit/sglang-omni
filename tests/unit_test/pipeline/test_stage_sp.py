...
def make_stage(*, follower=False, scheduler=None):
    ...
    stage = Stage(
        ...
        rank_endpoints={"decode": ("tcp://127.0.0.1:12345",)},
    )
...
def test_abort_drains_committed_work_and_acknowledges_terminal(follower, terminal):
    ...
    async def run():
        if follower:
            ...
            stage._on_abort("r", dispatch_id=1)
            await stage._execute(payload, dispatch_id=1)
        else:
        ...
        if follower and terminal == "error":
...
def test_abort_before_dispatch_does_not_fanout():
    ...
    stage._on_abort("r")
    asyncio.run(stage._execute(SimpleNamespace(request_id="r")))
...
def test_abort_listener_correlates_only_committed_work():
    async def run():
        ...
        stage._running = True
        await stage._abort_listener()
        assert aborts.empty()

        stage, work, aborts = make_stage()
        await stage._execute(SimpleNamespace(request_id="r"))
...
def test_sp_platform_environment_and_mps_identity(monkeypatch):
    ...
    with stage_workers._patched_spawn_env(worker):
...
def test_fanned_abort_preserves_collective_participation():
    ...
    async def run():
        await leader._execute(SimpleNamespace(request_id="r"))
        msg = work.get_nowait()
        leader._on_abort("r")
        follower._on_abort("r", dispatch_id=msg.dispatch_id)
        await follower._execute(msg.data, dispatch_id=msg.dispatch_id)
        threads = [
        ...
        try:
            for stage in (leader, follower):
                ...
                stage._acknowledge_terminal(terminal.request_id)
        finally:
            for stage in (leader, follower):
...
def test_existing_tp_specs_keep_factory_and_kv_rank_wiring():
    config = PipelineConfig(
        model_path="m",
        stages=[
            StageConfig(
