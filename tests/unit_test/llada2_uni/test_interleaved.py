...
def test_collector_orders_frames_by_frame_index() -> None:
    state = LLaDA2UniPipelineState(
        stream_state={
            "interleaved_done": True,
    ...
    final_payload = _payload(state)
    def frame_payload(index: int) -> StagePayload:
        return StagePayload(
            request_id="request",
            request=final_payload.request,
            data={
    ...
    collector = InterleavedCollectorScheduler()
    collector._receive("request", frame_payload(2))
    collector._receive("request", final_payload)
    collector._receive("request", frame_payload(1))
...
def test_interleaved_pipeline_topologies_keep_thinker_and_decoder_disjoint():
    ...
    parallel = LLaDA2UniInterleavedPipelineConfig.model_validate(raw)
...
@pytest.mark.parametrize("name", ["llada2_uni", "llada2_uni_interleaved"])
def test_shipped_yaml_uses_current_config_resolver(name):
    from sglang_omni.config.manager import ConfigManager

    root = Path(__file__).resolve().parents[3]
    config = ConfigManager.from_file(
        str(root / "examples" / "configs" / f"{name}.yaml")
    ).config
