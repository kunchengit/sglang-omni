...
from sglang_omni.models.llada2_uni.request_builders import merge_image_tokens_for_thinker
...
def test_cfg_batched_model_fn():
    ...
    def model(**kwargs):
        calls.append(kwargs)
        return ([positive_latent, positive_latent * 2, negative_latent, negative_latent * 2],)
    ...
    expected *= min(
        1.0,
        torch.linalg.vector_norm(positive_latent)
...
def test_decode_pipeline(decode_probe):
    ...
    torch.testing.assert_close(
        observed.ids, torch.tensor([[1, 1, 2, 2, 1, 1, 2, 2]])
    )
...
def preprocessor():
    ...
    processor._image_processor = None
    return processor


def test_edit_preprocess_merge_extract_and_decode(preprocessor, monkeypatch):
    generation = {
        "source_image_tokens": {
    ...
    state = LLaDA2UniPipelineState.from_dict(
        asyncio.run(preprocessor(payload)).data
    )
    ...
    assert [tid for tid in prompt_ids if tid >= IMAGE_TOKEN_OFFSET] == [
        IMAGE_TOKEN_OFFSET + token_id for token_id in range(10, 16)
    ]

    state.thinker_out = {
        "output_ids": [IMAGE_TOKEN_OFFSET + token_id for token_id in range(10, 16)]
        + [2]
    }
    tokens, h, w, params = extract_image_vq_tokens(state)
    assert tokens == list(range(10, 16))
    assert (h, w) == (2, 3)
    ...
    class FakeDecoder:
        def decode_to_bytes(self, token_ids, h, w, format="PNG", **kwargs):
            decoded.append((token_ids, h, w, kwargs))
            image = Image.new("RGB", (w * 16, h * 16))
            buffer = io.BytesIO()
            image.save(buffer, format=format)
            return buffer.getvalue()

    monkeypatch.setattr(decoder_module, "LLaDA2ImageDecoder", FakeDecoder)
    data = FakeDecoder().decode_to_bytes(tokens, h, w, seed=params["seed"])
    assert data.startswith(b"\x89PNG")
    assert decoded == [(tokens, h, w, {"seed": 42})]
